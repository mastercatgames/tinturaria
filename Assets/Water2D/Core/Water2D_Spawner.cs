namespace Water2D
{
    ...
    public class Water2D_Spawner : MonoBehaviour
    {
        ...
        [Header("Custom variables and actions (Added by Augusto Polonio)")]
        /// <summary>
        /// The dynamic changes can be apply ?.
        /// </summary>
		...
		public Color OriginalWaterFillColor;
		public Color OriginalWaterStrokeColor;
        ...
        IEnumerator loop(Vector3 _pos, Vector2 _initSpeed, int count = -1, float delay = 0f, bool waitBetweenDropSpawn = true)
        {
            ...
            while (true)
            {
                for (int i = 0; i < WaterDropsObjects.Length; i++)
                {
                    ...
                    //Commented By Augustio Polonio - Esse código reseta a posição das bolinhas
		...
		#region Edited by Augusto Polonio
        ...
        //Edited by Augusto Polonio
        //Save the current AlphaCutout Material
        void OnEnable()
        {
            OriginalWaterFillColor = WaterMaterial.GetColor("_Color");
            OriginalWaterStrokeColor = WaterMaterial.GetColor("_StrokeColor");
        }
        // generate a message when the game shuts down
        void OnDestroy()
        {
            SetWaterColor(OriginalWaterFillColor, OriginalWaterStrokeColor);
        }
		#endregion
    }
